       ...
       PlugInUIVals *ui_vals)
{
  ...
  adj = gimp_scale_entry_new(GTK_TABLE(table), 0, row++,
                             "Dummy 2:", SCALE_WIDTH, SPIN_BUTTON_WIDTH,
                             vals->dummy2, 0, 200, 1, 10, 0,
                             TRUE, 0, 0,
                             "Dummy scale entry 2", NULL);
  g_signal_connect(adj, "value_changed",
                   G_CALLBACK(gimp_int_adjustment_update),
                   &vals->dummy2);

  adj = gimp_scale_entry_new(GTK_TABLE(table), 0, row++,
                             "Dummy 3:", SCALE_WIDTH, SPIN_BUTTON_WIDTH,
                             vals->dummy3, -100, 100, 1, 10, 0,
                             TRUE, 0, 0,
                             "Dummy scale entry 3", NULL);
  g_signal_connect(adj, "value_changed",
                   G_CALLBACK(gimp_int_adjustment_update),
                   &vals->dummy3);
  ...
  frame = gimp_frame_new("A Random Seed Entry");
  gtk_box_pack_start(GTK_BOX(main_vbox), frame, FALSE, FALSE, 0);
  gtk_widget_show(frame);
  ...
  coordinates = gimp_coordinates_new(unit, "%p", TRUE, TRUE, SPIN_BUTTON_WIDTH,
                                     ...
                                     "Width:", drawable->width, xres,
                                     1, GIMP_MAX_IMAGE_SIZE,
                                     0, drawable->width,

                                     "Height:", drawable->height, yres,
                                     1, GIMP_MAX_IMAGE_SIZE,
                                     0, drawable->height);
  ...
  frame = gimp_frame_new("Image and Drawable Menu Examples");
  gtk_box_pack_start(GTK_BOX(main_vbox), frame, FALSE, FALSE, 0);
  gtk_widget_show(frame);
  ...
  gimp_table_attach_aligned(GTK_TABLE(table), 0, row++,
                            "RGB Images:", 0.0, 0.5, combo, 1, FALSE);
